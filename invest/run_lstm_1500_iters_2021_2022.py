"""
LSTM Sequential Model - 1500 iterations for 2021-2022
Testing if more iterations improve convergence and stock selection
"""
...
# Parameters - 1500 iterations to test convergence
GAMMA = 0.3
LEARNING_RATE = 0.001
TRAINING_STEPS = 1500  # DOUBLE THE ORIGINAL
NUM_TIMESTEPS = 7
LSTM_HIDDEN_DIM = 64
NUM_LSTM_LAYERS = 2
DROPOUT_RATIO = 0.0
...
output_dir = f'{data_dir}lstm_1500iter_{datetime.now().strftime("%Y%m%d_%H%M%S")}/'
...
print("\n" + "="*80)
print("TESTING 1500 ITERATIONS ON 2021-2022")
...
def train_lstm_model(train_data):
    ...
    for step in range(TRAINING_STEPS):
        ...
        # Progress updates - more frequent for longer training
        if (step + 1) % 250 == 0:
...
def evaluate_on_test_data(model, test_file_path):
    ...
    with torch.no_grad():
        ...
        # Also report concentration metrics
        top10_weight = torch.topk(weights.flatten(), 10).values.sum().item()
        max_weight = weights.max().item()
    
    return {
        'return': total_return * 100,  # Convert to percentage
        'num_stocks': num_stocks,
        'top10_weight': top10_weight * 100,  # Top 10 stocks weight
        'max_weight': max_weight * 100  # Largest position
    }

# Process 2021 and 2022
all_results = {}
...
for year in [2021, 2022]:
    ...
    for month in sorted(available_months.keys()):
        ...
        # Skip months before May for 2021 (to match original LSTM test)
        ...
        # Train model with 1500 iterations
...
# Save results
results_file = f'{output_dir}lstm_1500iter_results.json'
...
report_file = f'{output_dir}lstm_1500iter_report.md'
with open(report_file, 'w') as f:
    f.write("# LSTM Results - 1500 Iterations vs 750 Iterations\n\n")
    f.write("## Configuration\n")
    f.write("- 1500 training iterations (2x original), γ=0.3\n")
    f.write("- LSTM: 64 hidden dims, 2 layers, gradient clipping (max_norm=5.0)\n")
    f.write("- Transaction costs: 0.15% per trade\n\n")
    
    f.write("## Purpose\n")
    f.write("Testing if more iterations improve convergence and stock selection\n\n")
    
    for year_str, results in all_results.items():
        if 'summary' in results:
            s = results['summary']
            f.write(f"\n## {year_str} Results (1500 iterations)\n\n")
            ...
            f.write("| Month | Return | Stocks>1% | Top10 Weight | Max Position |\n")
            f.write("|-------|--------|-----------|--------------|-------------|\n")
            if 'trades' in results:
                for trade in results['trades']:
                    f.write(f"| {trade['actual_trading_month']} | {trade['return']:+.2f}% | {trade['num_stocks']} | {trade['top10_weight']:.1f}% | {trade['max_weight']:.1f}% |\n")
