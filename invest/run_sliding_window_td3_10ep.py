#!/usr/bin/env python3
"""
Run TD3+10-episode sliding window experiment for comparison with DQN variants.
"""
...
def run_td3_10_episode_experiment():
    """Run the TD3+10-episode sliding window experiment with logging."""
    ...
    stdout_log = f"{log_dir}/sliding_window_td3_10ep_{timestamp}_stdout.log"
    stderr_log = f"{log_dir}/sliding_window_td3_10ep_{timestamp}_stderr.log"
    combined_log = f"{log_dir}/sliding_window_td3_10ep_{timestamp}_combined.log"
    
    print(f"=== TD3+10-Episode Sliding Window Experiment ===")
    print(f"Starting at: {datetime.now()}")
    print(f"Algorithm: TD3 with twin networks, tau updates, policy delay")
    print(f"Episodes: 10 per window")
    print(f"Comparison with DQN 5-episode and DQN 10-episode")
...
def main():
    print(f"=== TD3+10-Episode Sliding Window Experiment Started ===")
    ...
    # Create experiment with TD3 algorithm and 10 episodes per window
    experiment = SlidingWindowExperiment(
        target_eval_days=240,
        training_window_size=265,
        eval_window_size=60,
        window_shift=60,
        base_exp_id="sliding_window_td3_10ep"
    )
    
    # Print plan
    experiment.print_experiment_plan()
    
    print(f"\\n=== Starting TD3+10-Episode Experiment ===")
    print(f"Expected TD3 advantages:")
    ...
    print(f"  - Better handling of continuous action spaces")
    start_time = time.time()
    
    try:
        # Run the TD3 experiment
        results = experiment.run_full_experiment(algorithm='td3')
        
        total_time = time.time() - start_time
        print(f"\\n=== TD3+10-Episode Experiment Complete ===")
        ...
        if 'summary' in results and 'evaluation_performance' in results['summary']:
            ...
            print(f"\\n=== TD3+10-Episode Results Summary ===")
            ...
            print(f"\\n=== Comparison with Baselines ===")
            print(f"DQN 5-episode baseline:")
            baseline_5ep_eval = -12.56
            baseline_5ep_train = -7.23
            print(f"  Evaluation: {{baseline_5ep_eval:.2f}}% (baseline)")
            print(f"  Training: {{baseline_5ep_train:.2f}}% (baseline)")
            
            print(f"TD3 10-episode results:")
            print(f"  Evaluation: {{eval_perf['mean_return_pct']:.2f}}% ({{eval_perf['mean_return_pct'] - baseline_5ep_eval:+.2f}}% vs 5ep)")
            print(f"  Training: {{train_perf['mean_return_pct']:.2f}}% ({{train_perf['mean_return_pct'] - baseline_5ep_train:+.2f}}% vs 5ep)")
            
            # Volatility comparison
            print(f"\\nVolatility Analysis:")
            baseline_5ep_std = 12.94
            print(f"  DQN 5ep std: {{baseline_5ep_std:.2f}}%")
            print(f"  TD3 10ep std: {{eval_perf['std_return_pct']:.2f}}% ({{eval_perf['std_return_pct'] - baseline_5ep_std:+.2f}}% change)")
            
            if eval_perf['std_return_pct'] < baseline_5ep_std:
                print(f"  ✅ TD3 shows more consistent performance")
            else:
                print(f"  ⚠️  TD3 shows higher volatility")
        
        print(f"\\n=== TD3+10-Episode Experiment Finished Successfully ===")
        print(f"End timestamp: {{datetime.now()}}")
        
    except Exception as e:
        print(f"\\n=== TD3+10-Episode Experiment Failed ===")
        print(f"Error: {{str(e)}}")
        print(f"End timestamp: {{datetime.now()}}")
        import traceback
        traceback.print_exc()
...
"""
    ...
    script_path = f"{log_dir}/sliding_window_td3_10ep_runner_{timestamp}.py"
    ...
    print(f"\nStarting TD3+10-episode background process...")
    ...
    print(f"\n=== TD3+10-Episode Process Complete ===")
    print(f"Return code: {return_code}")
    print(f"End time: {end_time}")
    
    if return_code == 0:
        print("✅ TD3+10-episode experiment completed successfully!")
    else:
        print("❌ TD3+10-episode experiment failed!")
