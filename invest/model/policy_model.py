...
class PolicyModel(torch.nn.Module):
    def __init__(self, dropout_ratio=0.0, num_conv_filters = 32, hidden_dim=47, shuffle_dict=None, num_tickers = -1, device=torch.device('cuda')): #num_conv_filters = 16, hidden_dim=32):
        ...
        self.prev_delta_net = torch.nn.Sequential(
            torch.nn.Linear(self.num_tickers, hidden_dim),
            torch.nn.Tanh(),
        )
        self.prev_action_net = torch.nn.Sequential(
            torch.nn.Linear(self.num_tickers, hidden_dim),
            torch.nn.Tanh(),
        )
        self.prev_sharpe_net = torch.nn.Sequential(
            torch.nn.Linear(1, hidden_dim), 
            torch.nn.Tanh(),
        )
        # convolution from price features 
    ...
    def forward(self, state, tickers, return_acts = False):
        ...
        # sum up activations from features, delta, action, sharpe nets 
        #(this will be a broadcast of the same d, a, s values across multiple tickers in the features batch) 
        x = x + d + a + s 
...
class PolicyModelV0(torch.nn.Module):
    """
    version of the policy model before replay buffer was implemented 
    """
    def __init__(self, dropout_ratio=0.0, num_conv_filters = 32, hidden_dim=47, shuffle_dict=None, num_tickers = -1, device=torch.device('cuda')): #num_conv_filters = 16, hidden_dim=32):
        super(PolicyModel, self).__init__()
        self.num_conv_filters = num_conv_filters
