"""
LSTM Sequential Model - SCALED UP ARCHITECTURE
3 LSTM layers, 64 hidden dimensions, 750 iterations
Testing if deeper architecture improves performance
"""
...
# SCALED UP ARCHITECTURE PARAMETERS
GAMMA = 0.3
LEARNING_RATE = 0.001
TRAINING_STEPS = 750
NUM_TIMESTEPS = 7
LSTM_HIDDEN_DIM = 64  # Same as 2-layer version
NUM_LSTM_LAYERS = 3   # INCREASED from 2
DROPOUT_RATIO = 0.0
NUM_CONV_FILTERS = 32

# Data configuration
data_dir = '/home/ubuntu/code/angle_rl/invest/data/'
output_dir = f'{data_dir}lstm_750iter_3layer_{datetime.now().strftime("%Y%m%d_%H%M%S")}/'
...
print(f"=" * 80)
print(f"SCALED UP LSTM ARCHITECTURE TEST")
print(f"Configuration: gamma={GAMMA}, {TRAINING_STEPS} iterations")
print(f"LSTM: hidden_dim={LSTM_HIDDEN_DIM}, layers={NUM_LSTM_LAYERS} (INCREASED)")
...
def train_lstm_model(train_data):
    """Train 3-LAYER LSTM model on prepared data."""
    model = LSTMSequentialModel(
        num_conv_filters=NUM_CONV_FILTERS,
        hidden_dim=47,
        lstm_hidden_dim=LSTM_HIDDEN_DIM,  # 64
        num_lstm_layers=NUM_LSTM_LAYERS,   # 3 layers
...
for year in [2021, 2022, 2023, 2024, 2025]:
    ...
    for month in sorted(available_months.keys()):
        ...
        print(f"\n{year}-{month:02d}: Training with 750 iterations (3-layer architecture)")
    ...
    if results['monthly_returns']:
        ...
        print(f"\n{year} Summary (3-Layer Architecture):")
...
# Save results
results_file = f'{output_dir}lstm_750iter_3layer_results.json'
...
report_file = f'{output_dir}lstm_3layer_comparison.md'
with open(report_file, 'w') as f:
    f.write("# LSTM 3-Layer Architecture Results\n\n")
    ...
    f.write("| **3** | **64** | **~85K** | **Scaled Up (current)** |\n\n")
    
    f.write("## Performance Summary\n\n")
    f.write("| Year | 3-Layer-64dim | 2-Layer-64dim | 1-Layer-32dim | Sequential | Best |\n")
    f.write("|------|---------------|---------------|---------------|------------|------|\n")
    
    # Known results for comparison
    prev_results = {
        '2021': {'2-layer': '+12.28%', '1-layer': 'N/A', 'sequential': '-28.96% (H2)'},
        '2022': {'2-layer': '-17.16%', '1-layer': 'N/A', 'sequential': '-16.22%'},
        '2023': {'2-layer': '+5.31%', '1-layer': '-11.26%', 'sequential': '+14.00%'},
        '2024': {'2-layer': '+19.95%', '1-layer': '+11.28%', 'sequential': '+406.17%'},
        '2025': {'2-layer': '-20.17%', '1-layer': 'N/A', 'sequential': 'N/A'}
    }
    ...
    for year_str, results in all_results.items():
        if 'summary' in results:
            ...
            if year_str == '2024':
                ...
                best = 'Sequential' if s['annual_return'] < 14.00 else '3-Layer'
    ...
    for year_str, results in all_results.items():
        if 'summary' in results:
            f.write(f"\n### {year_str} Monthly Returns (3-Layer)\n\n")
    ...
    f.write("\n## Key Findings\n\n")
    f.write("1. **Depth Impact**: Compare 3-layer vs 2-layer performance\n")
    f.write("2. **Convergence**: Check if deeper model converges better/worse\n")
    f.write("3. **2024 Mystery**: Does 3-layer capture any of the +406%?\n")
    f.write("4. **Stability**: Compare monthly volatility across architectures\n")

print(f"Report saved to: {report_file}")
print("\n" + "="*80)
print("3-LAYER ARCHITECTURE TEST COMPLETE")
print("Compare with 1-layer, 2-layer, and Sequential results")
print("="*80)