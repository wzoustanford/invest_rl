"""
Run LSTM Sequential Model with gamma=0.3 for 2021 and 2022
Compares with existing Sequential Supervised results
"""
...
# Parameters matching the sequential supervised setup
GAMMA = 0.3
LEARNING_RATE = 0.001
TRAINING_STEPS = 750  # Match the 750 iterations from gamma comparison
NUM_TIMESTEPS = 7
LSTM_HIDDEN_DIM = 64
NUM_LSTM_LAYERS = 2
DROPOUT_RATIO = 0.0
...
output_dir = f'{data_dir}lstm_gamma03_{datetime.now().strftime("%Y%m%d_%H%M%S")}/'
...
def train_lstm_model(train_data):
    ...
    for step in range(TRAINING_STEPS):
        ...
        if (step + 1) % 100 == 0:
            ...
            print(f"  Step {step+1}/{TRAINING_STEPS}, Avg Loss: {avg_loss:.4f}")
...
# Run experiments for 2021 and 2022
results = {
    'config': {
...
for year in ['2021', '2022']:
    if results[year]['monthly_returns']:
        ...
        annual_return = np.prod([1 + r/100 for r in returns]) - 1
...
# Save results
results_file = f'{output_dir}lstm_gamma03_results.json'
...
comparison_table = """
| Year | Model | Gamma | Annual Return | Win Rate | Best Month | Worst Month |
|------|-------|-------|---------------|----------|------------|-------------|
"""

# Add existing results from the gamma comparison report
comparison_table += "| 2021 | Sequential | 0.1 | -42.39% | 42% | +7.85% | -20.65% |\n"
comparison_table += "| 2021 | Sequential | 0.3 | -16.22% | 33% | +16.14% | -18.64% |\n"
comparison_table += "| 2021 | Sequential | 0.5 | -7.33% | 42% | +43.75% | -17.29% |\n"

# Add LSTM results for 2021
if '2021' in results and 'summary' in results['2021']:
    s = results['2021']['summary']
    comparison_table += f"| 2021 | **LSTM** | **0.3** | **{s['annual_return']:.2f}%** | **{s['win_rate']*100:.0f}%** | **{s['best_month']:.2f}%** | **{s['worst_month']:.2f}%** |\n"

comparison_table += "| 2022 | Sequential | 0.1 | -19.57% | 33% | +13.19% | -18.37% |\n"
comparison_table += "| 2022 | Sequential | 0.3 | +14.00% | 42% | +56.98% | -16.19% |\n"
comparison_table += "| 2022 | Sequential | 0.5 | +27.98% | 60% | +61.59% | -15.20% |\n"

# Add LSTM results for 2022
if '2022' in results and 'summary' in results['2022']:
    s = results['2022']['summary']
    comparison_table += f"| 2022 | **LSTM** | **0.3** | **{s['annual_return']:.2f}%** | **{s['win_rate']*100:.0f}%** | **{s['best_month']:.2f}%** | **{s['worst_month']:.2f}%** |\n"
...
with open(comparison_file, 'w') as f:
    f.write("# LSTM vs Sequential Supervised Comparison\n\n")
    f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    f.write("## Configuration\n")
    f.write(f"- LSTM Hidden Dim: {LSTM_HIDDEN_DIM}\n")
    f.write(f"- LSTM Layers: {NUM_LSTM_LAYERS}\n")
    ...
    for year in ['2021', '2022']:
        if results[year]['trades']:
            f.write(f"\n### {year} Monthly Details (LSTM γ={GAMMA})\n")
            for trade in results[year]['trades']:
                f.write(f"- Month {trade['month']:02d}: {trade['return']:.2f}% ({trade['num_stocks']} stocks)\n")
