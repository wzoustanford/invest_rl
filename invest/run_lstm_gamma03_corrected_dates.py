"""
LSTM Sequential Model with gamma=0.3 for 2021 and 2022
CORRECTED DATE ALIGNMENT: test_data_start_date is 1 year before actual trading date
"""
...
output_dir = f'{data_dir}lstm_gamma03_corrected_{datetime.now().strftime("%Y%m%d_%H%M%S")}/'
...
print("\n" + "="*60)
print("IMPORTANT: Date Alignment Correction")
print("test_data_start_date in filename = Feature data date")
print("Actual trading date = Feature date + 1 year")
print("Example: test_data_start_date_2020_04_26 → Trading in 2021-04-26")
print("="*60 + "\n")
...
def find_files_for_year_month(target_trading_year, target_trading_month):
    """
    Find 7 consecutive files ending at the given trading year/month.
    CORRECTED: The trading year is 1 year after the test_data_start_date in filename.
    """
    target_files = []
    target_idx = None
    
    # We need to look for files with test_data_start_date that is 1 year BEFORE target trading date
    target_file_year = target_trading_year - 1
    target_file_month = target_trading_month
    ...
    for i, filepath in enumerate(all_files):
        ...
        if date_str:
            ...
            # Check if this file corresponds to our target trading date
            if file_year == target_file_year and file_month == target_file_month:
                ...
                if i >= 6:
                    ...
                    print(f"  Found file for trading {target_trading_year}-{target_trading_month:02d}:")
                    print(f"    File test_data_start_date: {date_str}")
                    print(f"    Actual trading date: {target_trading_year}-{target_trading_month:02d}")
...
results = {
    'config': {
        ...
        'num_lstm_layers': NUM_LSTM_LAYERS,
        'num_timesteps': NUM_TIMESTEPS,
        'date_alignment': 'CORRECTED - trading date is 1 year after test_data_start_date'
    },
    '2021': {
        'monthly_returns': [],
        'trades': []
...
print("=" * 60)
print("Processing 2021 TRADING YEAR (files from 2020)")
print("=" * 60)

for month in [2, 4, 6, 8, 10, 12]:  # Every 2 months
    print(f"\n2021-{month:02d} (Trading Date):")
    
    # Find training files - looking for 2020 dates since trading is in 2021
...
print("\n" + "=" * 60)
print("Processing 2022 TRADING YEAR (files from 2021)")
print("=" * 60)

for month in [2, 4, 6, 8, 10, 12]:  # Every 2 months
    print(f"\n2022-{month:02d} (Trading Date):")
    
    # Find training files - looking for 2021 dates since trading is in 2022
...
print("\n" + "=" * 60)
print("RESULTS SUMMARY (WITH CORRECTED DATE ALIGNMENT)")
print("=" * 60)
...
# Save results
results_file = f'{output_dir}lstm_gamma03_corrected_results.json'
...
comparison_table = """
# LSTM vs Sequential Supervised Comparison (Corrected Date Alignment)

## IMPORTANT: Date Alignment
- **test_data_start_date in filename**: Date of feature data (1 year of historical prices)
- **Actual trading date**: 1 year AFTER test_data_start_date
- **Example**: test_data_start_date_2020_04_26 → Trading happens in 2021-04-26
...
comparison_file = f'{output_dir}lstm_comparison_table_corrected.md'
