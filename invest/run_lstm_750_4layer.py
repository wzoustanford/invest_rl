"""
LSTM Sequential Model - 4-LAYER ARCHITECTURE
4 LSTM layers, 64 hidden dimensions, 750 iterations
Testing if even deeper architecture improves performance
"""
...
# 4-LAYER ARCHITECTURE PARAMETERS
GAMMA = 0.3
LEARNING_RATE = 0.001
TRAINING_STEPS = 750
NUM_TIMESTEPS = 7
LSTM_HIDDEN_DIM = 64  # Same as 2-layer and 3-layer versions
NUM_LSTM_LAYERS = 4   # INCREASED from 3
DROPOUT_RATIO = 0.0
NUM_CONV_FILTERS = 32

# Data configuration
data_dir = '/home/ubuntu/code/angle_rl/invest/data/'
output_dir = f'{data_dir}lstm_750iter_4layer_{datetime.now().strftime("%Y%m%d_%H%M%S")}/'
...
print(f"=" * 80)
print(f"4-LAYER LSTM ARCHITECTURE TEST")
print(f"Configuration: gamma={GAMMA}, {TRAINING_STEPS} iterations")
print(f"LSTM: hidden_dim={LSTM_HIDDEN_DIM}, layers={NUM_LSTM_LAYERS} (MAXIMUM DEPTH)")
...
def train_lstm_model(train_data):
    """Train 4-LAYER LSTM model on prepared data."""
    model = LSTMSequentialModel(
        num_conv_filters=NUM_CONV_FILTERS,
        hidden_dim=47,
        lstm_hidden_dim=LSTM_HIDDEN_DIM,  # 64
        num_lstm_layers=NUM_LSTM_LAYERS,   # 4 layers
...
for year in [2021, 2022, 2023, 2024, 2025]:
    ...
    for month in sorted(available_months.keys()):
        ...
        print(f"\n{year}-{month:02d}: Training with 750 iterations (4-layer architecture)")
    ...
    if results['monthly_returns']:
        ...
        print(f"\n{year} Summary (4-Layer Architecture):")
...
# Save results
results_file = f'{output_dir}lstm_750iter_4layer_results.json'
...
report_file = f'{output_dir}lstm_4layer_comparison.md'
with open(report_file, 'w') as f:
    f.write("# LSTM 4-Layer Architecture Results\n\n")
    f.write("## Architecture Comparison\n")
    f.write("| Layers | Hidden Dim | Parameters | Purpose |\n")
    f.write("|--------|------------|------------|----------|\n")
    f.write("| 1 | 32 | ~15K | Simplified (tested 2023-2024) |\n")
    f.write("| 2 | 64 | ~50K | Original (tested all years) |\n")
    f.write("| 3 | 64 | ~85K | Scaled Up (tested all years) |\n")
    f.write("| **4** | **64** | **~120K** | **Maximum Depth (current)** |\n\n")
    
    f.write("## Performance Summary\n\n")
    f.write("| Year | 4-Layer-64dim | 3-Layer-64dim | 2-Layer-64dim | 1-Layer-32dim | Sequential | Best |\n")
    f.write("|------|---------------|---------------|---------------|---------------|------------|------|\n")
    
    # Known results for comparison
    prev_results = {
        '2021': {'3-layer': '-5.16%', '2-layer': '+12.28%', '1-layer': 'N/A', 'sequential': '-28.96% (H2)'},
        '2022': {'3-layer': '-23.61%', '2-layer': '-17.16%', '1-layer': 'N/A', 'sequential': '-16.22%'},
        '2023': {'3-layer': '+42.17%', '2-layer': '+5.31%', '1-layer': '-11.26%', 'sequential': '+14.00%'},
        '2024': {'3-layer': '+32.90%', '2-layer': '+19.95%', '1-layer': '+11.28%', 'sequential': '+406.17%'},
        '2025': {'3-layer': '+16.57%', '2-layer': '-20.17%', '1-layer': 'N/A', 'sequential': 'N/A'}
    }
    ...
    for year_str, results in all_results.items():
        if 'summary' in results:
            ...
            if year_str == '2024':
                ...
                if s['annual_return'] > 42.17:
                    best = '4-Layer'
                else:
                    best = '3-Layer'
            
            f.write(f"| {year_str} | **{s['annual_return']:+.2f}%** | "
                   f"{prev.get('3-layer', 'N/A')} | "
                   f"{prev.get('2-layer', 'N/A')} | "
                   f"{prev.get('1-layer', 'N/A')} | "
                   f"{prev.get('sequential', 'N/A')} | "
                   f"{best} |\n")
    
    f.write("\n## Monthly Details by Year\n\n")
    for year_str, results in all_results.items():
        if 'summary' in results:
            f.write(f"\n### {year_str} Monthly Returns (4-Layer)\n\n")
            f.write("| Month | Return | Stocks>1% | Top10 Weight | Max Position | Final Loss |\n")
            f.write("|-------|--------|-----------|--------------|--------------|------------|\n")
            if 'trades' in results:
                for trade in results['trades']:
                    month = trade['month']
                    loss_info = results.get('loss_histories', {}).get(month, {})
    ...
    f.write("\n## Key Findings\n\n")
    f.write("1. **Depth Impact**: Compare 4-layer vs 3-layer performance\n")
    f.write("2. **Convergence**: Check if deeper model converges better/worse\n")
    f.write("3. **Overfitting Risk**: Does 4-layer show signs of overfitting?\n")
    f.write("4. **Optimal Depth**: Is 3-layer or 4-layer the sweet spot?\n")

print(f"Report saved to: {report_file}")
print("\n" + "="*80)
print("4-LAYER ARCHITECTURE TEST COMPLETE")
print("Compare with 1-layer, 2-layer, 3-layer, and Sequential results")
print("="*80)