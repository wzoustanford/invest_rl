...
class AgentTrainer 
    ...
    def __init():
        ...
        self.meta_learner_trainer = MetaLearnerTrainer()
...
def train_RL_model_delay(
    ...
    #ranges += list(range(eval_start_date_idx, eval_end_date_idx_plus1))
        ...
        #for i in ranges:
            ...
            sharpe, mean_return, actual_return, stddev = compute_kday_returns(shuffled_series, action) 
        ...
        #3print(state['action'])
            ...
            # try to change sharpe to the actual gain so transaction cost is considered 
            sharpe = (X - state['X']) / state['X'] 
            ...
            if i % action_update_interval != 0: 
                ...
                policy_pooled_acts = prev_state['policy_pooled_acts']
            state = {
            ...
            if i > start_date_idx + B + 1 or e > 0: 
            ...
            if i > start_date_idx + B + 1 and i < end_date_idx_plus1 or e > 0: 
                ...
                print('Eval T-step: {} [{}/{} ({:.0f}%)]. X: {:.5f}. Loss (value mse): {:.5f}. Sharpe: {:.5f}. gamma*Q: {:.5f}. Q_prev: {:.5f}. Mean Return: {:.5f}. Actual Return: {:.5f}.'.format(
                    i - start_date_idx, 
                    i - start_date_idx, 
                    end_date_idx_plus1 - start_date_idx,
                    100. * (i - start_date_idx) / (end_date_idx_plus1 - start_date_idx), 
